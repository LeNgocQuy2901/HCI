    ...
    (set, get) => ({
      ...
      addProgress: (cardId, userId) => {
        ...
        if (!store.progress.has(key)) {
          ...
          const next = new Map(store.progress);
          next.set(key, {
            cardId,
            userId,
            status: "new",
      ...
      markCardUnderstood: (cardId, userId) => {
        const store = get();
        const next = new Map(store.understoodCards);
        next.set(`${userId}:${cardId}`, true);
        set({ understoodCards: next });
        void get().syncToServer(userId);
      },
      ...
      startLesson: (lessonId, userId) => {
        ...
        const next = new Map(store.lessonProgress);
        next.set(key, {
          lessonId,
          userId,
      ...
      updateStreak: (userId) => {
        ...
        const next = new Map(store.streaks);
        next.set(userId, {
          current: newCurrent,
          longest: newLongest,
