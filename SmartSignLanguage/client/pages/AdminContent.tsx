...
export default function AdminContent() {
  ...
  const [metadataForm, setMetadataForm] = useState<ContentSignMetadata>(
    blankMetadata(),
  );
  const [lessonIds, setLessonIds] = useState<string[]>([]);
  const [quizQuestions, setQuizQuestions] = useState<ContentQuizQuestion[]>([]);
  const [quizForm, setQuizForm] = useState<ContentQuizQuestion>(
    blankQuizQuestion(),
  );
  ...
  const getImpactRank = (item: SignCompleteness) => {
    ...
    const lessonRank = itemLessonIds.some((id) => id.startsWith("beginner-"))
      ...
      : item.lessonCount > 0
        ? 1
        : 2;
    const categoryRank = ["greeting", "family", "colors"].includes(item.category)
      ? 0
      : 1;
    const wordRank = [
      "alphabet",
  ...
  const filteredCompleteness = useMemo(() => {
    ...
    return completeness.filter((item) => {
      const matchesQuery =
        !normalized ||
      ...
      return matchesQuery && matchesStatus && matchesPriority && matchesPublishStatus;
    }).sort((a, b) => {
      const impactDelta = getImpactRank(a) - getImpactRank(b);
      if (impactDelta !== 0) return impactDelta;
      return a.score - b.score;
    });
  }, [completeness, lessonIdsBySign, priorityFilter, publishStatusFilter, query, statusFilter]);
  ...
  const loadContent = async () => {
    ...
    const nextSelectedId =
      selectedId || signsData.signs?.[0]?.id || completenessData.completeness?.[0]?.signId;
    if (nextSelectedId) {
  ...
  const saveQuizQuestion = async () => {
    ...
    if (!signId) {
      toast({ title: "Save sign first", description: "A quiz question needs a sign ID." });
      return;
    }
    ...
    if (!response.ok) {
      toast({ title: "Quiz save failed", description: "Check the question, options, and answer." });
      return;
    }
  ...
  const deleteQuizQuestion = async (id: string) => {
    ...
    if (!response.ok) {
      toast({ title: "Delete failed", description: "Quiz question was not deleted." });
      return;
    }
  ...
  const updateLessonStatus = async (
    ...
    if (!response.ok) {
      toast({ title: "Status update failed", description: "Lesson status was not changed." });
      return;
    }
      ...
      <div className="container max-w-7xl mx-auto py-10 px-4 space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">
              Edit vocabulary content, assign signs to lessons, and track data
              quality gaps before expanding the dataset.
            </p>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <Metric label="Missing metadata" value={summary.missingMetadata} />
          <Metric label="Quality score" value={`${summary.averageScore}%`} />
        </div>
            ...
            <div className="grid gap-3 sm:grid-cols-[1fr_180px_180px_180px] xl:grid-cols-1">
              ...
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="low-score">Low completeness score</SelectItem>
                  <SelectItem value="ready-to-publish">Ready to publish</SelectItem>
                  <SelectItem value="cannot-publish">Cannot publish</SelectItem>
                </SelectContent>
              </Select>
              ...
              <Select value={priorityFilter} onValueChange={setPriorityFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="high-impact">High impact first</SelectItem>
                ...
                filteredCompleteness.map((item) => (
                  <div
                    ...
                    className={`w-full text-left rounded-md border p-3 transition-colors hover:bg-muted ${
                      item.signId === selectedId ? "border-primary bg-muted" : ""
                    }`}
                  >
            ...
            <Tabs defaultValue="sign">
              ...
              <TabsContent value="sign" className="space-y-5 pt-4">
                {selectedCompleteness && (
                  <div className="rounded-md border p-4 space-y-3">
                    ...
                    <Progress value={selectedCompleteness.score} className="h-2" />
                  </div>
                )}
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <Field label="Word">
                    <Input
                      value={signForm.word}
                      onChange={(event) =>
                        setSignForm({ ...signForm, word: event.target.value })
                      }
                      placeholder="Hello"
                    />
                  </Field>
                  <Field label="Category">
                    <Select
                      ...
                      onValueChange={(value) =>
                        setSignForm({ ...signForm, category: value as Category })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                  ...
                  <div className="rounded-md border p-4 space-y-2">
                    ...
                    {selectedCompleteness.publishBlockers.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {selectedCompleteness.publishBlockers.map((blocker) => (
                          <Badge key={blocker} variant="outline">
                            {blocker}
                          </Badge>
                        ))}
                      ...
                      <p className="text-sm text-muted-foreground">
                        This sign has metadata, video, lesson assignment, and at
                        least 2 active quiz questions.
                      </p>
                    )}
                  </div>
                )}
                ...
                <Field label="Video URL">
                  <div className="flex gap-2">
                    <Input
                      ...
                      onChange={(event) =>
                        setSignForm({ ...signForm, videoUrl: event.target.value })
                      }
                ...
                <Field label="Example sentence">
                  <Textarea
                    ...
                    onChange={(event) =>
                      setSignForm({ ...signForm, example: event.target.value })
                    }
                    rows={3}
                  />
                </Field>
              ...
              <TabsContent value="media" className="space-y-5 pt-4">
                <div className="rounded-md border p-4 space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    This video is used in learning cards, video-to-word quiz,
                    and quiz previews.
                  </p>
                </div>
                <Field label="Video URL">
                ...
                <div className="rounded-md border p-4 space-y-3">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" className="gap-2" onClick={startNewQuizQuestion}>
                        <Plus className="h-4 w-4" />
                        Add Question
                      </Button>
                    </div>
                ...
                {previewQuiz && quizQuestions.length > 0 && (
                  <div className="rounded-md border p-4 space-y-3">
                    ...
                    {quizQuestions.slice(0, 3).map((question) => (
                      <div key={question.id} className="rounded-md bg-muted p-3 space-y-2">
                        <div className="flex flex-wrap gap-2">
                          <Badge variant="secondary">
                      ...
                      <div>
                        <div className="flex flex-wrap gap-2">
                          ...
                          <Badge variant="outline">
                            {lessons.find((lesson) => lesson.id === question.lessonId)?.title ||
                              question.lessonId}
                          </Badge>
                          <Badge variant={question.status === "active" ? "secondary" : "outline"}>
                            {question.status}
                          </Badge>
                        </div>
                ...
                <div className="rounded-md border p-4 space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <Field label="Question type">
                      ...
                      >
                        ...
                        <SelectContent>
                          {Object.entries(quizTypeLabels).map(([key, label]) => (
                    ...
                    <Field label="Lesson">
                      <Select
                        value={quizForm.lessonId || lessonIds[0] || lessons[0]?.id || ""}
                        onValueChange={(value) =>
                  ...
                  <Field label="Question">
                    <Textarea
                      ...
                      onChange={(event) =>
                        setQuizForm({ ...quizForm, question: event.target.value })
                      }
                      rows={3}
                    />
                  </Field>
                    ...
                    <Field label="Difficulty">
                      <Select
                        ...
                        onValueChange={(value) =>
                          setQuizForm({
                            ...quizForm,
                            difficulty: value as ContentQuizQuestion["difficulty"],
                          })
                        }
                      >
                        <SelectTrigger>
              ...
              <TabsContent value="lessons" className="space-y-4 pt-4">
                <p className="text-sm text-muted-foreground">
                  Assign this sign to one or more lessons so it appears in lesson
                  flow and quiz coverage.
                </p>
                      ...
                      <label className="flex items-start gap-3">
                        ...
                        <span>
                          ...
                          <span className="block text-xs text-muted-foreground">
                            {lesson.level} · {categoryLabels[lesson.category]}
                          </span>
                        </span>
                      </label>
                      <Select
                        value={lesson.status}
                        onValueChange={(value) =>
                          updateLessonStatus(lesson.id, value as ContentPublishStatus)
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
